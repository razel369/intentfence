...
async function findOwnedOffer(title) {
...
for (const desired of offers) {
  const existing = await findOwnedOffer(desired.title);
  if (existing) {
    await marketplaceJson(`/api/v1/offers/${existing._id}`, {
      method: "PATCH",
      body: JSON.stringify({ ...desired, offerType: undefined }),
    });
    results.push({ title: desired.title, offer_id: existing._id, action: "updated" });
  } else {
    const created = await marketplaceJson("/api/v1/offers", {
      method: "POST",
      body: JSON.stringify(desired),
    });
    results.push({ title: desired.title, offer_id: created.offerId, action: "created" });
  }
}
